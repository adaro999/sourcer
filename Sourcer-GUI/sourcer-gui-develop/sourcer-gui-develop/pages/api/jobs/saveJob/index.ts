...
const saveJob = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    const { companyId, userId, unique_name } = parseToken(req);
    const convertedParams = new URLSearchParams(req?.body).toString();
    const { data } = await instance.post<string>(`/internal/Sourcing/Savejobs?${convertedParams}&cid=${companyId}&rid=${userId || unique_name}`);
...
export default use(validToken(), saveJob);
